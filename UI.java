...
public class UI {
	...
	public void hoverSettings(Point p) {
		...
		} else if(BMTabBounds.contains(p)) {
			hoveredButton = BMTabBounds;
		} else if(PMTabBounds.contains(p)) {
			hoveredButton = PMTabBounds;
		} else if(SITabBounds.contains(p)) {
			hoveredButton = SITabBounds;
		}
		hoverCursor();
	}
	...
	private void drawSILeaderboard() {
		...
		LeaderboardSorter ls = LeaderboardReader.loadLeaderboardFromFile(BMGamePanel.DATA_FILE);//CHANGE TO SI DATA FILE
	...
	private void drawPMLeaderboard() {
		LeaderboardPlayer p = null;
		LeaderboardSorter ls = LeaderboardReader.loadLeaderboardFromFile(BMGamePanel.DATA_FILE);//CHANGE TO PMS
		for (int i = 0; i < Math.min(20, ls.getPlayersSize()); i++) {
            p = ls.getPlayer(i);
