...
public class SpaceInvaders extends JPanel implements ActionListener, KeyListener, MouseListener {
    ...
    // Resource paths
    public static final String FONT_PATH = "fonts/space_invaders.ttf";
    ...
    public SpaceInvaders() {
        ...
        try {
            titleScreenBgImg = ImageIO.read(new File(IMAGE_PATH + "titlescreenbackground.png"));
            titleScreenLogoImg = ImageIO.read(new File(IMAGE_PATH + "titlescreen.png"));
        } catch (IOException e) {
        ...
        try {
            ...
            AudioInputStream ais = AudioSystem.getAudioInputStream(new File(SOUND_PATH + "themesong.wav"));
    ...
    private void loadResources() {
        // Load images
        String[] imgNames = {"ship", "mystery", "enemy1_1", "enemy1_2", "enemy2_1", "enemy2_2", "enemy3_1", "enemy3_2", "explosionblue", "explosiongreen", "explosionpurple", "laser", "enemylaser", "background"};
        for (String name : imgNames) {
            try {
                images.put(name, ImageIO.read(new File(IMAGE_PATH + name + ".png")));
            } catch (IOException e) {
                if (!name.equals("background")) System.err.println("Image not found: " + name);
            }
        }
        try {
            images.put("background", ImageIO.read(new File(IMAGE_PATH + "background.jpg")));
        } catch (IOException e) {
        ...
        // Load font
        try {
            gameFont = Font.createFont(Font.TRUETYPE_FONT, new File(FONT_PATH)).deriveFont(24f);
        } catch (Exception e) {
            gameFont = new Font("Arial", Font.BOLD, 24);
        }
        try {
            images.put("gameoverbg", ImageIO.read(new File(IMAGE_PATH + "titlescreenbackground.png")));
        } catch (IOException e) {
