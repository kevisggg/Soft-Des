...
public class BMCollisionChecker{
	...
	public void checkTile(BMEntity entity) {
		...
		int entityLeftCol = entityLeft/BMGamePanel.tileSize;
		int entityRightCol = entityRight/BMGamePanel.tileSize;
		int entityTopRow = entityTop/BMGamePanel.tileSize;
		int entityBottomRow = entityBottom/BMGamePanel.tileSize;
		
		int tile1, tile2;
		
		switch(entity.direction) {
		case "up":
			entityTopRow = (entityTop - entity.speed)/BMGamePanel.tileSize;
			...
			break;
		case "down":
			entityBottomRow = (entityBottom + entity.speed)/BMGamePanel.tileSize;
			...
			break;
		case "left":
			entityLeftCol = (entityLeft - entity.speed)/BMGamePanel.tileSize;
			tile1 = tileMgr.getMapTileNum(entityLeftCol, entityTopRow);//TOP LEFT
			...
			break;
		case "right":
			entityRightCol = (entityRight + entity.speed)/BMGamePanel.tileSize;
	...
	public boolean checkTileExp(int x, int y, boolean empty) {
		...
		if(tileMgr.getTileCollision(tile) == true) {//if colliding tile
			if(tile == 2) {//DESTROY TILE AND RANDOMIZE PU DROP
				...
				int drop=randomDrop();
				if(drop!=0) {
					asset.setObject(drop, x*BMGamePanel.tileSize, y*BMGamePanel.tileSize);
				}
			...
			for(int i=0;i<obj.size();i++) {
				if(x==(obj.get(i).getX())/BMGamePanel.tileSize && y==(obj.get(i).getY())/BMGamePanel.tileSize) {
					obj.get(i).addHits();
	...
	public int randomDrop() {
		...
		int i = random.nextInt(100)+1;//30% chance to drop power up, 15% chance is capacity, 15% is range
		if(i>=1 && i<=15) {//
			drop = 1;
