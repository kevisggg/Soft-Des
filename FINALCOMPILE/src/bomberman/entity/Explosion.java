...
public class Explosion{
	...
	public Explosion(int x, int y) {
		...
		expActive = true;
		expStart = false;
		collisionBox = new Rectangle(x+5,y+6,BMGamePanel.tileSize-10,BMGamePanel.tileSize-13);
		//hasHitPlayer = false;
		exp = setupImage("/BM/objects/explosion.png");
	}
	...
	public BufferedImage setupImage(String path) {
		...
		try {
			...
			image = s.scale(BMGamePanel.tileSize, BMGamePanel.tileSize, image);
		}catch(IOException e) {
			e.printStackTrace();
		}
		return image;
	...
	public void draw(Graphics2D g2, BMGamePanel gp){
		if(expActive) {
