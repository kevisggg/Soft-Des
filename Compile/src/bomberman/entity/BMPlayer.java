...
public class BMPlayer extends BMEntity implements PlayerInterface{
	...
	public BMPlayer(BomberManGamePanel gp, KeyHandler keyH, BMCollisionChecker colCheck) {
		super(gp, colCheck);
		this.keyH = keyH;
		name = "Player";
	...
	public void setDefaultVal() {
		worldX = BomberManGamePanel.tileSize;
		worldY = BomberManGamePanel.tileSize;
			...
			if(keyH.getSpace()==true) {
				...
				if(bombsPlaced < bombCnt && !gp.getTileManager().getTileCollision(collisionTile)) {
					...
					for(Bomb b: gp.getBombs()) {
						if((b.getX()+ collisionBox.x+5)/BomberManGamePanel.tileSize == (worldX + collisionBox.x+5)/BomberManGamePanel.tileSize && (b.getY()+ collisionBox.x+10)/BomberManGamePanel.tileSize == (worldY+collisionBox.y+10)/BomberManGamePanel.tileSize) {
